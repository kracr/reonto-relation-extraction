...
def id2word(word2id):
    """
    Returns a dictionary that maps index to words, given a dictionary that maps words to index.
    """
    return {v: k for k, v in word2id.items()}


def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_FORNEW_BERT(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    with open ("entity2id.txt","r") as frrr:
     entity2id=json.load(frrr)  
    graphs_to_process = []
    sentences = np.zeros((len(graphs), max_sent_len),dtype="int32")
    ...
    for index,g in enumerate(graphs):
        #print(g)
        if(len(g)>0):
         eid,sen,e1,len1,len2,e2,l1,l2,rel,edgeSet = (g).strip().split("|")
         parse = ast.literal_eval(edgeSet)
         sen=str(sen)[1:-1]
         entity1=entity2id[e1]
         entity2=entity2id[e2]
         tok=word_tokenize(sen)
        if len(parse) > 0:
            if len(parse) <= MAX_EDGES_PER_GRAPH:
                graphs_to_process.append(g)
            else:
                continue # here we discard these data points
                for i in range(0, len(parse), MAX_EDGES_PER_GRAPH):
                    graphs_to_process.append({"tokens": tok, "edgeSet": parse[i:i+ MAX_EDGES_PER_GRAPH]})
        #print(g)
        try:    
            ...
            continue
        token_ids = embedding_utils.get_idx_sequence(tok, word2idx)
        #print(token_ids)
        idx2word = id2word(word2idx)
        textword=[]
        ...
        sentences=textword
        for i in parse:
                pos2id[tuple(i['left'])] = entity2id[e1]
                pos2id[tuple(i['right'])]=entity2id[e2]
        entity_pair_instance = []
        for j, edge in enumerate (parse[:MAX_EDGES_PER_GRAPH]):
            new_j = calculate_order_conversion(j,2)
            entity_matrix[index, new_j, :len(token_ids)] = \
                [m for _, m in graph_utils.get_entity_indexed_vector(token_ids, edge, mode="mark-bi")]
            _, property_kbid, _ =  entity2id[e1],property2idx[rel],entity2id[e2]
    ...
    #print(entity_pair)             
    return [sentences_matrix, entity_matrix, y_matrix, entity_cnt, entity_pair,sentences,id2word]


def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_FORNEW(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    with open ("entity2id.txt","r") as frrr:
     entity2id=json.load(frrr)  
    graphs_to_process = []
    sentences_matrix = np.zeros((len(graphs), max_sent_len), dtype="int32")
...
def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_ADEFULL(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    """
    :param graphs:
    :param word2idx:
    :param property2idx:
    ...
    for g in graphs:
        ...
        if(len(g)>0):
         #print(g)
         eid,sen,res,len1,len2,e2,l1,l2,rel,edgeSet = (g).strip().split("|")
         parse = ast.literal_eval(edgeSet)
         tok = word_tokenize(sen)
         #print("tok0:",tok[0],e2)
         sen=str(sen)[1:-1]
         entity1=entity2id[tok[0]]
         entity2=entity2id[e2]
        if len(parse) > 0:
            if len(parse) <= MAX_EDGES_PER_GRAPH:
                graphs_to_process.append(g)
...
def to_indices_with_relative_positions_and_pcnn_mask_and_entity_pair_Biorel(graphs, word2idx, property2idx, max_sent_len, position2idx, **kwargs):
    d={}
    final={}
    with open("/data/entity.txt") as file:
         for line in file:
            line=line.split("\t")
            remspace=removelines(line[1])
            entity2id[line[0]]=removelines(line[1])
            
    
    #num_edges = len([e for g in graphs for e in g['edgeSet']])
    sentences_matrix = np.zeros((len(graphs), max_sent_len), dtype="int32")
    entity_matrix = np.zeros((len(graphs), 2, max_sent_len), dtype="int8")
    y_matrix = np.zeros(len(graphs), dtype="int16")
    pcnn_mask = np.zeros((len(graphs), 3, max_sent_len), dtype="float32")
    index = 0
    ...
    for g in tqdm.tqdm(graphs, ascii=True):
        ...
        d["right"]=[]
        d["kbID"]=g["relation"]
        d ["left"].append(g["head"]["split_start"])
        d["right"].append(g["tail"]["split_start"])
               
        final["left"]=(d["left"])
        final["right"]=(d["right"])
        final["kbID"]=(d["kbID"])
                 
        d["edgeSet"].append(final)
        g.update({"edgeSet":[final]})
        tok=word_tokenize(g["sentence"])
        g.update({"tokens":tok})
        #print(g["edgeSet"])          
        entity_cnt.append(2) 
        for ed in g["edgeSet"]:                   
         pos2id[tuple(ed['left'])] = g["head"]["CUI"]
         pos2id[tuple(ed["right"])]=g["tail"]["CUI"]
...
def to_indices_with_relative_positions_and_pcnn_mask_and_entity_pair_Biorel_Biobert(graphs, word2idx, property2idx, max_sent_len, position2idx, **kwargs):
    d={}
    final={}
    with open("/data/entity.txt") as file:
         for line in file:
    ...
    for g in tqdm.tqdm(graphs, ascii=True):
        ...
        for edge in g["edgeSet"]:
            ...
            entity_pair_instance.append((pos2id[tuple(edge['left'])], pos2id[tuple(edge['right'])]))
        entity_pair += entity_pair_instance
    return [sentences_matrix, entity_matrix, y_matrix, pcnn_mask, entity_pair,sentences,id2word]


def to_indices_with_relative_positions_and_entity_pair_Biorel(graphs, word2idx, property2idx, max_sent_len, position2idx, **kwargs):
    d={}
    final={}
    with open("/data/entity.txt") as file:
         for line in file:
    ...
    for g in tqdm.tqdm(graphs, ascii=True):
        ...
        for edge in g["edgeSet"]:
            ...
            entity_pair_instance.append((pos2id[tuple(edge['left'])], pos2id[tuple(edge['right'])]))
        entity_pair += entity_pair_instance
        #print(sentences_matrix,entity_matrix,y_matrix,entity_pair)

    return [sentences_matrix, entity_matrix, y_matrix, entity_pair]

def to_indices_with_relative_positions_Biorel(graphs, word2idx, property2idx, max_sent_len, position2idx, **kwargs):
    d={}
    final={}
    with open("L/data/entity.txt") as file:
         for line in file:
            line=line.split("\t")
            remspace=removelines(line[1])
            entity2id[line[0]]=removelines(line[1])
    ...
    for g in tqdm.tqdm(graphs, ascii=True):
        ...
        for edge in g["edgeSet"]:
            ...
            y_matrix[index] = property_kbid
            index += 1
    #print(sentences_matrix,entity_matrix,y_matrix)
    return [sentences_matrix, entity_matrix, y_matrix]




def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_entity_pair_Biorel(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    d={}
    final={}
    with open("/data/entity.txt") as file:
         for line in file:
...
def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_Biorel(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    """
    :param graphs:
    :param word2idx:
    :param property2idx:
    ...
    for index, g in enumerate(tqdm.tqdm(graphs,ascii=True)):
        ...
        for j, edge in enumerate(g["edgeSet"][:MAX_EDGES_PER_GRAPH]):
            ...
            entity_matrix[index, new_j, :len(token_ids)] = \
                [m for _, m in graph_utils.get_entity_indexed_vector(token_ids, edge, mode="mark-bi")]
            _, property_kbid, _ = graph_utils.edge_to_kb_ids_Bio(edge, g)
            #print(_, property_kbid, _) 
            #print(_,property_kbid, _)
            #property_kbid = property2idx.get(property_kbid, property2idx[embedding_utils.unknown])
            property_kbid=property2idx[property_kbid]
            #print(property_kbid,"last")
            y_matrix[index, new_j] = property_kbid
    entity_cnt = np.array(entity_cnt, dtype=np.int32)        
    # print(sentences_matrix)
    # print(entity_matrix)
...
def to_indices_with_real_entities_and_entity_nums_with_vertex_padding_ADE(graphs, word2idx, property2idx, max_sent_len, mode='train', **kwargs):
    """
    :param graphs:
    :param word2idx:
    :param property2idx:
...
def to_indices_with_relative_positions_and_pcnn_mask_and_entity_pair_ADE(graphs, word2idx, property2idx, max_sent_len, position2idx, **kwargs):
    with open ("entity2id.txt","r") as frrr:
     entity2id=json.load(frrr)  
    num_edges = len([g for g in graphs])
...
def to_indices_with_relative_positions_ADE(graphs, word2idx, property2idx, max_sent_len, position2idx,**kwargs):
    #entity()
    #print(property2idx)
    with open ("entity2id.txt","r") as frrr:
     entity2id=json.load(frrr)  
