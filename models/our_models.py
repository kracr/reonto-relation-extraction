...
class GPGNN_ONTOLOGY_BERT(nn.Module):
   
    def __init__(self, p,embeddings, max_sent_len, n_out):
        super(GPGNN_ONTOLOGY_BERT, self).__init__()
        ...
        self.bert = AutoModel.from_pretrained('bert-base-uncased', local_files_only=False)
    ...
    def forward(self, sentence_input, entity_markers, num_entities,sentences,id2word):
        expanded_sentence_input = torch.transpose(sentence_input.expand(MAX_EDGES_PER_GRAPH, sentence_input.size()[0], self.max_sent_len), 0, 1)
        print(sentences)
        # Encode sentences with BioBERT
        textword=[]
        for x in sentences:
            word = idx2word.get(idx)
            textword.append(word)
        bert_input = self.tokenizer(textword, padding=True, truncation=True, max_length=self.max_sent_len, return_tensors='pt')
        ...
        word_embeddings = bert_output.unsqueeze(1).repeat(1, MAX_EDGES_PER_GRAPH, 1, 1)
        ...
        # Encode the setntences with BioBERT
...
class GPGNN_ONTOLOGY(nn.Module):
    """
    Our model
    """

    def __init__(self, p, embeddings, max_sent_len, n_out):
        super(GPGNN_ONTOLOGY, self).__init__()
