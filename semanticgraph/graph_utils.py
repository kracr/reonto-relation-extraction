...
def get_entity_indexed_vector_ADE(l1,l2,tokens, edge, mode="mark"):
    """
    ...
    if mode == "bio-bi":
        ...
        #print(left,right)
        for i,t in enumerate(tokens):
          #print(t,token_to_entity_distance(left,i))
          #print(token_to_entity_distance(right,i))
...
def edge_to_kb_ids_Bio(edge, g):
    ...
    left_kbid = g["head"]["word"]
 
    right_kbid = g["tail"]["word"]
    property_kbid = kbid_empty if 'kbID' not in edge else edge['kbID']
    #print(left_kbid,property_kbid,right_kbid)
